...
def dsi_remap (dsi_data, df_mapper_dsi_to_dmf, df_mapper_col_headers ) : 
  ...
  log.debug("... df_cols_to_keep : \n%s", pformat(df_cols_to_keep))
  ...
  df_cols_to_drop = [ h for h in df_cols if h not in df_cols_to_keep and h != "oid_dsi" ]
  log.debug("... df_cols_to_drop : \n%s", pformat(df_cols_to_drop))
  ...
  remapper_dict["oid_dsi"] = "oid_dsi"
  log.debug("... remapper_dict : \n%s", pformat(remapper_dict) )
